...
public class TwoYearRaiseComparator extends SimpleComparator {
	...
	public int compare(String o1, String o2) {
		...
		double result;
		try {
			result = getTwoYearRaise(o1) - getTwoYearRaise(o2);
			if(result > 0)
				return 1;
			else if(result < 0)
				return -1;
	...
	private double getTwoYearRaise(String fundCode) throws IOException {
		...
		if(datas.length<365*2)
			compareValue = Double.parseDouble(datas[datas.length-1][3]);
		else
			compareValue = Double.parseDouble(datas[365*2][3]);
